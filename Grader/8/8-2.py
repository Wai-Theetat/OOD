...
class Solution:
	def main(self):
		...
		print()
		if self.check_same_tree(Tree1.get_root(), Tree2.get_root()):
			print("Same Tree")
		else:
			print("Different Tree")

	def check_same_tree(self, Tree1 : AVLNode, Tree2 : AVLNode):
		if Tree1 is None and Tree2 is None : return True
		if Tree1 is None or	 Tree2 is None : return False
  
		return ((Tree1.get_data() == Tree2.get_data()) and
				self.check_same_tree(Tree1.get_left(), Tree2.get_left()) and
				self.check_same_tree(Tree1.get_right(), Tree2.get_right()))
